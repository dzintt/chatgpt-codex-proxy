...
func TestContinuationInputHistoryNormalizesEmptyAssistantMessageReplay(t *testing.T) {
	t.Parallel()
	...
	if history[1].Type != "" {
		t.Fatalf("history[1].Type = %q, want empty replayable role message type", history[1].Type)
	}
	if history[1].Role != "assistant" {
		t.Fatalf("history[1].Role = %q, want assistant", history[1].Role)
	}
	if history[1].Phase != "output" {
		t.Fatalf("history[1].Phase = %q, want output", history[1].Phase)
	}
	if len(history[1].Content) != 1 {
		t.Fatalf("history[1].Content = %#v, want one empty output_text part", history[1].Content)
	}
	if history[1].Content[0].Type != "output_text" || history[1].Content[0].Text != "" {
		t.Fatalf("history[1].Content[0] = %#v, want empty output_text", history[1].Content[0])
	}

	replayed := continuationInputItemsToCodex(history)
	payload, err := json.Marshal(replayed[1])
	if err != nil {
	...
	if decoded["content"] != "" {
		t.Fatalf("decoded content = %#v, want explicit empty string", decoded["content"])
	}
}
